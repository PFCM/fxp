...
// fractional bits, representing numbers between 0 and {{.Max}}.
{{if .Comment -}}
// {{.Comment}}
{{end -}}
type {{.Typename}} uint8
...
func Test{{.Typename}}Ops(t *testing.T) {
	...
	test := func(t *testing.T, f func({{.Typename}}, {{.Typename}}) {{.Typename}}, h func(float64, flaot64) float64) {
		t.Helper()
		for range 10 {
			shuffle()
			for i, a := range as {
				...
				got := f(a, b)
				if got != want {
					t.Errorf("(%v, %v) = %v (%x), want: %v (%x)", a, b, got, got, want, want)
				}
			}
		}
	}
	t.Run("SAdd", func(t *testing.T) {
		test(t, {{.Typename}}.SAdd, func(a, b float64) {
			x := a + b
			if x < {{.Min}} {
				return {{.Min}}
			}

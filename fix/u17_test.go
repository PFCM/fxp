...
func TestU17Ops(t *testing.T) {
	...
	test := func(t *testing.T, name string, f func(U17, U17) U17, h func(float64, float64) float64) {
		t.Helper()
		for range 10 {
			shuffle()
			for i, a := range as {
				b := bs[i]
				af := U17ToFloat[float64](a)
				bf := U17ToFloat[float64](b)
				wantf := h(af, bf)
				want := U17FromFloat(wantf)
				got := f(a, b)
	...
	t.Run("SAdd", func(t *testing.T) {
		test(t, "SAdd", U17.SAdd, func(a, b float64) float64 {
			x := a + b
			if x < 0 {
				return 0
			}
	...
	t.Run("SSub", func(t *testing.T) {
		test(t, "SSub", U17.SSub, func(a, b float64) float64 {
			x := a - b
			if x < 0 {
				return 0
			}
	...
	t.Run("SMul", func(t *testing.T) {
		test(t, "SMul", U17.SMul, func(a, b float64) float64 {
			x := a * b
			if x < 0 {
				return 0
			}

...
func TestU80FloatRoundTrip(t *testing.T) {
	for i := 0; i < 256; i++ {
		u := U80(i)
		f := U80ToFloat[float64](u)
		u2 := U80FromFloat(f)
		if u != u2 {
...
func TestU80Ops(t *testing.T) {
	...
	test := func(t *testing.T, name string, f func(U80, U80) U80, h func(float64, float64) float64) {
		t.Helper()
		for range 10 {
			shuffle()
			for i, a := range as {
				b := bs[i]
				af := U80ToFloat[float64](a)
				bf := U80ToFloat[float64](b)
				wantf := h(af, bf)
				want := U80FromFloat(wantf)
				got := f(a, b)
	...
	t.Run("SAdd", func(t *testing.T) {
		test(t, "SAdd", U80.SAdd, func(a, b float64) float64 {
			x := a + b
			if x < 0 {
				return 0
			}
	...
	t.Run("SSub", func(t *testing.T) {
		test(t, "SSub", U80.SSub, func(a, b float64) float64 {
			x := a - b
			if x < 0 {
				return 0
			}
	...
	t.Run("SMul", func(t *testing.T) {
		test(t, "SMul", U80.SMul, func(a, b float64) float64 {
			x := a * b
			if x < 0 {
				return 0
			}

...
func TestU71Ops(t *testing.T) {
	...
	test := func(t *testing.T, name string, f func(U71, U71) U71, h func(float64, float64) float64) {
		t.Helper()
		for range 10 {
			shuffle()
			for i, a := range as {
				b := bs[i]
				af := U71ToFloat[float64](a)
				bf := U71ToFloat[float64](b)
				wantf := h(af, bf)
				want := U71FromFloat(wantf)
				got := f(a, b)
	...
	t.Run("SAdd", func(t *testing.T) {
		test(t, "SAdd", U71.SAdd, func(a, b float64) float64 {
			x := a + b
			if x < 0 {
				return 0
			}
	...
	t.Run("SSub", func(t *testing.T) {
		test(t, "SSub", U71.SSub, func(a, b float64) float64 {
			x := a - b
			if x < 0 {
				return 0
			}
	...
	t.Run("SMul", func(t *testing.T) {
		test(t, "SMul", U71.SMul, func(a, b float64) float64 {
			x := a * b
			if x < 0 {
				return 0
			}

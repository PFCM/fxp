...
func TestU26FloatRoundTrip(t *testing.T) {
	for i := 0; i < 256; i++ {
		u := U26(i)
		f := U26ToFloat[float64](u)
		u2 := U26FromFloat(f)
		if u != u2 {
...
func TestU26Ops(t *testing.T) {
	...
	test := func(t *testing.T, name string, f func(U26, U26) U26, h func(float64, float64) float64) {
		t.Helper()
		for range 10 {
			shuffle()
			for i, a := range as {
				b := bs[i]
				af := U26ToFloat[float64](a)
				bf := U26ToFloat[float64](b)
				wantf := h(af, bf)
				want := U26FromFloat(wantf)
				got := f(a, b)
	...
	t.Run("SAdd", func(t *testing.T) {
		test(t, "SAdd", U26.SAdd, func(a, b float64) float64 {
			x := a + b
			if x < 0 {
				return 0
			}
	...
	t.Run("SSub", func(t *testing.T) {
		test(t, "SSub", U26.SSub, func(a, b float64) float64 {
			x := a - b
			if x < 0 {
				return 0
			}
	...
	t.Run("SMul", func(t *testing.T) {
		test(t, "SMul", U26.SMul, func(a, b float64) float64 {
			x := a * b
			if x < 0 {
				return 0
			}
